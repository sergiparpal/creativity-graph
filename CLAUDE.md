...
| `/kg-generate` | `kg-generator` | Run the discovery mechanisms (`kg_generate`: bridge/seed/compression/regroup/transplant/ensemble/periphery), phrase/name them, write `hypothesized`/`unverified` via `kg_propose`/`kg_operate` — generation is offensive, never metric-gated. Candidates carry an advisory `convergence` count (distinct mechanisms agreeing) — a grounding-queue ranking prior only, harness-gated, never a verdict |
...
| `/kg-query` | — | `kg_context` (budgeted, grounding-aware; grounded `items[]` vs separate `hypotheses[]`) + structural reads (incl. `kg_explain_path` — the grounded-only associative chain with its advisory `leap`); answers cite all three axes and report falsification counters |
