...
allowed-tools: Task, Bash, mcp__creativity-graph__kg_scrub, mcp__creativity-graph__kg_metrics, mcp__creativity-graph__kg_context, mcp__creativity-graph__query_graph
---
...
engine's `scrub.py`, now exposed as the MCP tool `mcp__creativity-graph__kg_scrub`. It is real and wired into the
flow as **Step 0** of each section:

1. **Step 0 — scrub before egress.** Call `mcp__creativity-graph__kg_scrub(text=<section body>)` to get the
...
Task(
  ...
  prompt: """
You are extracting ONE section of the source document into the canon via mcp__creativity-graph__kg_write.
...
1. `mcp__creativity-graph__kg_metrics()` — reads the **canon** directly and returns
...
2. `mcp__creativity-graph__kg_context(budget=2000)` — this **lazily projects** (rebuilds the derived layer if
...
Optionally spot-check structure with `mcp__creativity-graph__query_graph(node_type="compression")` or
`mcp__creativity-graph__query_graph(epistemic_state="unverified", limit=50)` to eyeball the written nodes/edges.
